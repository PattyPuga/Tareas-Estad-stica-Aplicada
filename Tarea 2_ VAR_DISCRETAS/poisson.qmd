...
```{r}
# --- Parámetros ---
lambda_min <- 6  # llamadas por minuto

# a) Probabilidad de exactamente 4 llamadas en 1 minuto
prob_4 <- dpois(4, lambda=lambda_min)
cat("a) Probabilidad de recibir exactamente 4 llamadas en 1 minuto:", round(prob_4,4), "\n")

# b) Probabilidad de 0 llamadas en 30 segundos
lambda_30s <- lambda_min * 0.5
prob_0_30s <- dpois(0, lambda=lambda_30s)
cat("b) Probabilidad de no recibir llamadas en 30 segundos:", round(prob_0_30s,4), "\n")

# c) Número esperado de llamadas en 1 hora
lambda_60min <- lambda_min * 60
cat("c) Número esperado de llamadas en 1 hora:", lambda_60min, "\n")

# d) Probabilidad de recibir entre 8 y 12 llamadas en 2 minutos
lambda_2min <- lambda_min * 2
prob_8_12 <- sum(dpois(8:12, lambda=lambda_2min))
cat("d) Probabilidad de recibir entre 8 y 12 llamadas en 2 minutos:", round(prob_8_12,4), "\n")
...
tasa_defecto <- 0.015
n_productos <- 500
lambda <- n_productos * tasa_defecto

# a) Probabilidad de exactamente 8 defectos
prob_8 <- dpois(8, lambda)
cat("a) Probabilidad de exactamente 8 defectos:", round(prob_8,4), "\n")

# b) Probabilidad de 10 o más defectos
prob_10_mas <- 1 - ppois(9, lambda)
cat("b) Probabilidad de 10 o más defectos:", round(prob_10_mas,4), "\n")

# c) Número de productos para P(al menos 1 defecto)=0.95
n_requerido <- -log(0.05)/tasa_defecto
cat("c) Número de productos para P(al menos 1 defecto)=0.95:", round(n_requerido), "\n")

# d) Nueva probabilidad con tasa 1.2%
tasa_nueva <- 0.012
lambda_nueva <- n_productos * tasa_nueva
prob_10_mas_nueva <- 1 - ppois(9, lambda_nueva)
cat("d) Probabilidad de 10 o más defectos con nueva tasa:", round(prob_10_mas_nueva,4), "\n")
...
prob_3 <- dpois(3, lambda=lambda_hora)
cat("a) Probabilidad de 3 pacientes en 1 hora:", round(prob_3,4), "\n")

# b) Ningún paciente en 2 horas
lambda_2h <- lambda_hora * 2
prob_0_2h <- dpois(0, lambda=lambda_2h)
cat("b) Probabilidad de 0 pacientes en 2 horas:", round(prob_0_2h,4), "\n")

# c) Superar capacidad de 8 pacientes en 4 horas
lambda_4h <- lambda_hora * 4
prob_mas_8 <- 1 - ppois(8, lambda=lambda_4h)
cat("c) Probabilidad de superar 8 pacientes en 4 horas:", round(prob_mas_8,4), "\n")

# d) Capacidad mínima para probabilidad de saturación < 0.10
capacidad_min <- qpois(0.90, lambda=lambda_4h)
cat("d) Capacidad mínima para saturación <10%:", capacidad_min, "\n")
...
n_vect <- c(5, 10, 30, 50, 100, 500)

# Número de simulaciones de medias
n_sim <- 1000

# Función para calcular medias muestrales
calcular_medias <- function(n, n_sim, lambda){
  medias <- numeric(n_sim)
  for(i in 1:n_sim){
    muestra <- rpois(n, lambda)
    medias[i] <- mean(muestra)
  }
...
# Graficar histogramas de medias muestrales y densidad normal teórica
par(mfrow=c(3,2), mar=c(4,4,2,1))

for(n in n_vect){
  medias <- calcular_medias(n, n_sim, lambda)
  
  hist(medias, breaks=30, probability=TRUE, col="lightblue",
       main=paste("n =", n), xlab="Media muestral")
  
  # Normal teórica según TCL
  mu <- lambda
  sigma <- sqrt(lambda/n)
  x_vals <- seq(min(medias), max(medias), length=100)
...
hist(S, breaks=(min(S):(max(S)+1))-0.5, probability=TRUE,
     col="lightblue", main="Suma de Poisson X1+X2 vs Poisson(6.3)",
     xlab="X1 + X2")

# Distribución teórica Poisson(6.3)
x_vals <- 0:max(S)
lines(x_vals, dpois(x_vals, lambda=lambda1+lambda2), col="red", lwd=2)

legend("topright", legend=c("Simulación","Poisson teórica"),
       col=c("lightblue","red"), lwd=c(10,2), bty="n")

# Comparar medias y varianzas
cat("Media simulada:", mean(S), " | Teórica:", lambda1+lambda2, "\n")
cat("Varianza simulada:", var(S), " | Teórica:", lambda1+lambda2, "\n")
...
# -------------------------------------------------
# a) Generar 10,000 muestras de X1 y X2 y calcular suma
# -------------------------------------------------
lambda1 <- 3.5
lambda2 <- 2.8
n_sim <- 10000
...
S2 <- X1 + X2  # suma de 2 Poisson

# -------------------------------------------------
# b) Comparar estadísticas con Poisson(6.3)
# -------------------------------------------------
lambda_sum <- lambda1 + lambda2
S2_theo <- rpois(n_sim, lambda_sum)

cat("=== Suma de 2 Poisson ===\n")
cat("Media simulada:", mean(S2), " | Teórica:", lambda_sum, "\n")
cat("Varianza simulada:", var(S2), " | Teórica:", lambda_sum, "\n\n")

# Histograma comparativo
hist(S2, breaks=(min(S2):(max(S2)+1))-0.5, probability=TRUE,
     col=rgb(0,0,1,0.3), main="Suma de 2 Poisson vs Poisson(6.3)",
     xlab="X1+X2")
lines(0:max(S2), dpois(0:max(S2), lambda_sum), col="red", lwd=2)
legend("topright", legend=c("Simulación","Poisson teórica"),
       col=c(rgb(0,0,1,0.3),"red"), lwd=c(10,2), bty="n")

# -------------------------------------------------
# c) Extensión: suma de 4 variables Poisson independientes
# -------------------------------------------------
lambda3 <- 1.2
lambda4 <- 2.0
...
S4 <- X1 + X2 + X3 + X4
lambda_S4 <- lambda1 + lambda2 + lambda3 + lambda4

cat("=== Suma de 4 Poisson ===\n")
cat("Media simulada:", mean(S4), " | Teórica:", lambda_S4, "\n")
cat("Varianza simulada:", var(S4), " | Teórica:", lambda_S4, "\n\n")

# Histograma comparativo
hist(S4, breaks=(min(S4):(max(S4)+1))-0.5, probability=TRUE,
     col=rgb(0,1,0,0.3), main="Suma de 4 Poisson vs Poisson teórica",
     xlab="X1+X2+X3+X4")
lines(0:max(S4), dpois(0:max(S4), lambda_S4), col="red", lwd=2)
legend("topright", legend=c("Simulación","Poisson teórica"),
       col=c(rgb(0,1,0,0.3),"red"), lwd=c(10,2), bty="n")
...
# a) Probabilidad de degradación con 25 solicitudes
cap1 <- 25
prob_deg1 <- 1 - ppois(cap1, lambda)
cat("a) Probabilidad de degradación (C=25):", round(prob_deg1,4), "\n")

# b) Probabilidad de degradación con 50 solicitudes
cap2 <- 50
prob_deg2 <- 1 - ppois(cap2, lambda)
cat("b) Probabilidad de degradación (C=50):", round(prob_deg2,6), "\n")

# c) Capacidad óptima para P(degradación) < 1%
cap_opt <- qpois(0.99, lambda)
cat("c) Capacidad mínima para P(degradación)<1%:", cap_opt, "\n")

# d) Simulación de 1 hora (60 minutos)
n_minutos <- 60
visitantes <- rpois(n_minutos, lambda)
deg_minutos <- sum(visitantes > cap1)
pct_deg <- deg_minutos / n_minutos * 100
cat("d) Porcentaje de tiempo con degradación (simulación 60 min):", round(pct_deg,2), "%\n")

# Histograma de visitantes por minuto
hist(visitantes, breaks=0:max(visitantes), col="lightblue", main="Visitantes por minuto (1 hora)",
     xlab="Número de visitantes")
abline(v=cap1, col="red", lwd=2, lty=2)
legend("topright", legend=c("Capacidad del servidor"), col="red", lwd=2, lty=2)
...
I <- 12
prob_agotarse <- 1 - ppois(I, lambda)
cat("a) Probabilidad de agotarse (I=12):", round(prob_agotarse,4), "\n")

# b) Inventario para probabilidad < 5%
I_opt_prob <- qpois(0.95, lambda)
cat("b) Inventario para P(agotarse)<0.05:", I_opt_prob, "\n")

# c) Costo esperado diario por simulación
n_sim <- 100000
X <- rpois(n_sim, lambda)

inventario <- 12
costos <- c_a * inventario + c_h * pmax(inventario - X, 0) + c_s * pmax(X - inventario, 0)
costo_esperado <- mean(costos)
cat("c) Costo esperado diario (I=12): $", round(costo_esperado,2), "\n")

# d) Optimización de inventario para minimizar costo esperado
I_seq <- 5:20
costo_esperado_vec <- sapply(I_seq, function(I){
  costos <- c_a * I + c_h * pmax(I - X, 0) + c_s * pmax(X - I, 0)
  mean(costos)
})

I_opt <- I_seq[which.min(costo_esperado_vec)]
cat("d) Inventario óptimo para minimizar costo esperado:", I_opt, "\n")

# Graficar costo esperado vs inventario
plot(I_seq, costo_esperado_vec, type="b", pch=19, col="blue",
     xlab="Inventario inicial", ylab="Costo esperado", main="Costo esperado vs Inventario")
abline(v=I_opt, col="red", lty=2)
legend("topright", legend=c("Costo esperado","Inventario óptimo"), col=c("blue","red"), lty=c(1,2), pch=c(19,NA))
