...
mu <- 350        # temperatura media
sigma <- 5       # desviación estándar
n <- 1000        # número de mediciones en un día
LCL <- 340       # límite inferior de especificación
UCL <- 360       # límite superior de especificación

# --- 1. Simular un día de producción ---
temperaturas <- rnorm(n, mean = mu, sd = sigma)

# --- 2. Porcentaje fuera de especificación ---
fuera_spec <- sum(temperaturas < LCL | temperaturas > UCL) / n * 100
cat("Porcentaje de tiempo fuera de especificación:", round(fuera_spec, 2), "%\n")

# --- 3. Descentrando el proceso a mu = 355°C ---
mu_descentrado <- 355
temperaturas2 <- rnorm(n, mean = mu_descentrado, sd = sigma)
fuera_spec2 <- sum(temperaturas2 < LCL | temperaturas2 > UCL) / n * 100
cat("Porcentaje fuera de especificación con mu = 355°C:", round(fuera_spec2, 2), "%\n")

# --- 4. Qué sigma se necesita para tener 99.9% dentro de especificación ---
# Queremos P(LCL <= X <= UCL) = 0.999 para X ~ N(350, sigma^2)
prob_deseada <- 0.999
sigma_req <- (UCL - LCL) / (2 * qnorm((1 + prob_deseada)/2))
cat("Desviación estándar necesaria para 99.9% dentro de especificación:", round(sigma_req, 4), "°C\n")

# --- 5. Visualización (opcional) ---
hist(temperaturas, breaks = 30, probability = TRUE, col = "lightblue",
     main = "Simulación de Temperaturas (μ=350, σ=5)",
     xlab = "Temperatura (°C)")
abline(v = c(LCL, UCL), col = "red", lwd = 2, lty = 2)
legend("topright", legend = c("Límites de especificación"),
...
# Parámetros
mu <- 50000      # media de ventas
sigma <- 12000   # desviación estándar
n_meses <- 24    # meses a simular

# --- 1. Simular ventas de 24 meses ---
ventas <- rnorm(n_meses, mean = mu, sd = sigma)
ventas

# --- 2. Probabilidad de ventas superiores a 60,000 ---
prob_superior_60000 <- 1 - pnorm(60000, mean = mu, sd = sigma)
cat("Probabilidad de ventas > 60,000 pesos:", round(prob_superior_60000, 4), "\n")

# --- 3. Meses esperados con ventas < 30,000 ---
prob_inferior_30000 <- pnorm(30000, mean = mu, sd = sigma)
meses_inferior_30000 <- prob_inferior_30000 * n_meses
cat("Meses esperados con ventas < 30,000:", round(meses_inferior_30000, 2), "\n")

# --- 4. Rango intercuartílico ---
IQR_ventas <- IQR(ventas)
cat("Rango intercuartílico (IQR) de las ventas simuladas:", round(IQR_ventas, 2), "\n")

# --- 5. Porcentaje de meses con ventas > 45,000 ---
porcentaje_sobre_45000 <- mean(ventas > 45000) * 100
cat("Porcentaje de meses con ventas > 45,000:", round(porcentaje_sobre_45000, 2), "%\n")

# --- 6. Visualización opcional ---
hist(ventas, breaks = 10, col = "lightblue", main = "Simulación de Ventas Mensuales",
     xlab = "Ventas (pesos)", ylab = "Frecuencia")
abline(v = c(30000, 45000, 60000), col = "red", lwd = 2, lty = 2)
legend("topright", legend = c("30,000", "45,000", "60,000"), col = "red", lwd = 2, lty = 2)
...
# Parámetros
mu <- 1200       # media de horas
sigma <- 150     # desviación estándar
n_baterias <- 800

# --- 1. Simular la vida útil de 800 baterías ---
vidas <- rnorm(n_baterias, mean = mu, sd = sigma)

# --- 2. Garantía que cubre 95% de las baterías ---
# Queremos P(X <= garantia) = 0.95
garantia_95 <- qnorm(0.95, mean = mu, sd = sigma)
cat("Garantía que cubre 95% de las baterías:", round(garantia_95, 2), "horas\n")

# --- 3. Número esperado de fallas con garantía de 900 horas en 10,000 baterías ---
prob_falla <- pnorm(900, mean = mu, sd = sigma)
fallas_esperadas <- prob_falla * 10000
cat("Número esperado de baterías que fallan antes de 900 horas:", round(fallas_esperadas), "\n")

# --- 4. Comparar media y mediana de la simulación con valores teóricos ---
media_sim <- mean(vidas)
mediana_sim <- median(vidas)
cat("Media simulada:", round(media_sim, 2), "vs media teórica:", mu, "\n")
cat("Mediana simulada:", round(mediana_sim, 2), "vs mediana teórica (≈ media):", mu, "\n")

# --- 5. Percentil 10 de la distribución ---
percentil_10 <- qnorm(0.10, mean = mu, sd = sigma)
cat("Percentil 10 de la vida útil:", round(percentil_10, 2), "horas\n")

# --- 6. Visualización opcional ---
hist(vidas, breaks = 30, probability = TRUE, col = "lightblue",
     main = "Simulación de Vida Útil de Baterías",
     xlab = "Horas", ylab = "Densidad")
abline(v = c(garantia_95, 900, percentil_10), col = c("green", "red", "purple"), lwd = 2, lty = 2)
legend("topright", legend = c("Garantía 95%", "Garantía 900h", "Percentil 10"),
       col = c("green", "red", "purple"), lwd = 2, lty = 2)
...
# Parámetros
mu <- 0                   # media de errores
sigma <- 2.5^0.5           # desviación estándar (corrigiendo potencia si es sigma^2)
n <- 1000                  # número de mediciones

# --- 1. Simular 1000 mediciones ---
errores <- rnorm(n, mean = mu, sd = sigma)

# --- 2. Porcentaje de errores entre -5 y +5 ---
porc_entre_menos5_5 <- mean(errores >= -5 & errores <= 5) * 100
cat("Porcentaje de errores entre -5 y +5:", round(porc_entre_menos5_5, 2), "%\n")

# --- 3. Probabilidad de error mayor a 6 unidades en valor absoluto ---
prob_mayor6 <- mean(abs(errores) > 6) * 100
cat("Porcentaje de errores con |error| > 6:", round(prob_mayor6, 2), "%\n")

# --- 4. Porcentaje de mediciones "precisas" (|error| <= 3) ---
porc_preciso <- mean(abs(errores) <= 3) * 100
cat("Porcentaje de mediciones precisas (|error| <= 3):", round(porc_preciso, 2), "%\n")

# --- 5. Verificar simetría alrededor de 0 ---
cat("Media simulada de los errores:", round(mean(errores), 4), "\n")
cat("Mediana simulada de los errores:", round(median(errores), 4), "\n")
cat("Diferencia media - mediana (cercana a 0 indica simetría):", 
    round(mean(errores) - median(errores), 4), "\n")

# --- 6. Visualización opcional ---
hist(errores, breaks = 30, probability = TRUE, col = "lightblue",
     main = "Simulación de Errores de Medición",
     xlab = "Error", ylab = "Densidad")
abline(v = c(-5, 5, -3, 3, -6, 6), col = c("green","green","blue","blue","red","red"), lwd = 2, lty = 2)
legend("topright", legend = c("|error| <= 5","|error| <= 3","|error| > 6"),
       col = c("green","blue","red"), lwd = 2, lty = 2)
...
# --- 1. Simular puntuaciones ---
grupo_A <- rnorm(n, mean = mu_A, sd = sigma_A)
grupo_B <- rnorm(n, mean = mu_B, sd = sigma_B)

# --- 2. Porcentaje de estudiantes del Grupo A que supera la media del Grupo B ---
porc_A_sobre_B <- mean(grupo_A > mu_B) * 100
cat("Porcentaje de estudiantes del Grupo A que supera la media del Grupo B:", round(porc_A_sobre_B, 2), "%\n")

# --- 3. Probabilidad de que un estudiante del Grupo B tenga puntuación < 70 ---
prob_B_menor70 <- mean(grupo_B < 70) * 100
cat("Probabilidad de que un estudiante del Grupo B tenga puntuación < 70:", round(prob_B_menor70, 2), "%\n")

# --- 4. Comparación visual de distribuciones ---
hist(grupo_A, breaks = 15, col = rgb(0,0,1,0.5), xlim = c(40,110),
     main = "Distribuciones de Puntuaciones de Examen", xlab = "Puntuación", ylab = "Frecuencia")
hist(grupo_B, breaks = 15, col = rgb(1,0,0,0.5), add = TRUE)
legend("topright", legend = c("Grupo A", "Grupo B"), fill = c(rgb(0,0,1,0.5), rgb(1,0,0,0.5)))

# --- 5. Puntuación que delimita el 10% superior ---
percentil_90_A <- quantile(grupo_A, 0.9)
percentil_90_B <- quantile(grupo_B, 0.9)
cat("Puntuación mínima para el 10% superior del Grupo A:", round(percentil_90_A,2), "\n")
cat("Puntuación mínima para el 10% superior del Grupo B:", round(percentil_90_B,2), "\n")
