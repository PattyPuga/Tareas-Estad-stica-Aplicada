...
# Parámetros
p <- 0.02   # probabilidad de pieza defectuosa
n <- 10     # número de piezas inspeccionadas
k <- 2      # número de defectuosas buscadas

# --- a) Probabilidad de obtener una pieza no defectuosa ---
p_no_def <- 1 - p
cat("a) Probabilidad de pieza no defectuosa:", round(p_no_def, 4), "\n")

# --- b) Media y varianza de la distribución Bernoulli ---
media_bernoulli <- p
var_bernoulli <- p * (1 - p)
cat("b) Media Bernoulli:", round(media_bernoulli,4), "\n")
cat("   Varianza Bernoulli:", round(var_bernoulli,4), "\n")

# --- c) Probabilidad de encontrar exactamente 2 defectuosas en 10 piezas ---
prob_2_def <- dbinom(k, size = n, prob = p)
cat("c) Probabilidad de exactamente 2 defectuosas en 10 piezas:", round(prob_2_def,6), "\n")
...
```{r}
# --- Ejemplo Bernoulli: Respuesta aleatoria en examen ---

# Probabilidad de acierto
p <- 1/4

# --- a) Modelo Bernoulli ---
cat("a) X ~ Bernoulli(p =", p, ")\n")

# --- b) Probabilidades ---
cat("b) Probabilidad de acertar:", p, "\n")
cat("   Probabilidad de fallar:", 1-p, "\n")
...
```{r}
# --- Ejercicios Binomial: Lanzamiento de moneda ---
...
cat("a) Probabilidad de exactamente 8 caras:", round(prob_8,6), "\n")

# b) Probabilidad de al menos 12 caras
prob_mayor12 <- sum(dbinom(12:15, size = n, prob = p))
cat("b) Probabilidad de al menos 12 caras:", round(prob_mayor12,6), "\n")
...
```{r}
# --- Ejercicio Binomial: Efectividad de medicamento ---

# Parámetros
n <- 20
p <- 0.85

# a) Probabilidad de exactamente 18 pacientes curados
prob_18 <- dbinom(18, size = n, prob = p)
cat("a) Probabilidad de exactamente 18 pacientes curados:", round(prob_18,6), "\n")

# b) Probabilidad de que se curen entre 15 y 19 pacientes
prob_15_19 <- sum(dbinom(15:19, size = n, prob = p))
cat("b) Probabilidad de que se curen entre 15 y 19 pacientes:", round(prob_15_19,6), "\n")

# c) Número esperado de pacientes curados
esperado <- n * p
cat("c) Número esperado de pacientes curados:", esperado, "\n")
...
# Simulación
X <- rbinom(num_sim, size = n, prob = p)
Y <- rbinom(num_sim, size = m, prob = p)
Suma <- X + Y

# Comparar media y varianza
cat("Media simulada de X+Y:", mean(Suma), "\n")
cat("Varianza simulada de X+Y:", var(Suma), "\n")

cat("Media teórica de Binomial(n+m,p):", (n+m)*p, "\n")
cat("Varianza teórica de Binomial(n+m,p):", (n+m)*p*(1-p), "\n")

# Histograma con distribución teórica
hist(Suma, breaks = (0:(n+m+1))-0.5, probability = TRUE,
     col = "lightblue", main = "Suma de Binomiales",
     xlab = "X + Y", ylab = "Probabilidad")

# Superponer probabilidad teórica
points(0:(n+m), dbinom(0:(n+m), size=n+m, prob=p), col="red", pch=16)
legend("topright", legend=c("Simulación","Teórica"), col=c("lightblue","red"), pch=c(15,16))
