...
```{r}
# --- Simulación: Distribución Ji-cuadrada (χ²) ---

set.seed(123)

# Grados de libertad a analizar
df_values <- c(5, 20, 50)

# Tamaños de muestra
sample_sizes <- seq(100, 10000, by = 100)

# Configuración de gráficos
par(mfrow = c(3, 3))  # 3 distribuciones × (media, varianza, histograma)

for (df in df_values) {
  
  # --- 1. Inicializar vectores para guardar resultados ---
  mean_samples <- numeric(length(sample_sizes))
  var_samples <- numeric(length(sample_sizes))
  
  # --- 2. Calcular medias y varianzas muestrales ---
  for (i in seq_along(sample_sizes)) {
    n <- sample_sizes[i]
    muestra <- rchisq(n, df = df)
    mean_samples[i] <- mean(muestra)
    var_samples[i] <- var(muestra)
  }
  
  # --- 3. Medias y varianzas teóricas ---
  mean_theoretical <- df
  var_theoretical <- 2 * df
  
  # --- 4. Gráfica de convergencia de la media ---
  plot(sample_sizes, mean_samples, type = "l", col = "blue", lwd = 2,
       main = bquote(paste("Convergencia de la media (", chi^2, "(", .(df), "))")),
       xlab = "Tamaño de muestra", ylab = "Media muestral")
  abline(h = mean_theoretical, col = "red", lwd = 2, lty = 2)
  legend("bottomright", legend = c("Media muestral", "Media teórica"),
         col = c("blue", "red"), lwd = 2, bty = "n")
  
  # --- 5. Gráfica de convergencia de la varianza ---
  plot(sample_sizes, var_samples, type = "l", col = "darkgreen", lwd = 2,
       main = bquote(paste("Convergencia de la varianza (", chi^2, "(", .(df), "))")),
       xlab = "Tamaño de muestra", ylab = "Varianza muestral")
  abline(h = var_theoretical, col = "red", lwd = 2, lty = 2)
  legend("bottomright", legend = c("Varianza muestral", "Varianza teórica"),
         col = c("darkgreen", "red"), lwd = 2, bty = "n")
  
  # --- 6. Histograma para la muestra más grande ---
  muestra_final <- rchisq(max(sample_sizes), df = df)
  hist(muestra_final, breaks = 30, probability = TRUE, col = "lightblue",
       border = "white", 
       main = bquote(paste("Histograma vs PDF teórica (", chi^2, "(", .(df), "))")),
       xlab = "Valores", ylab = "Densidad")
  
  curve(dchisq(x, df = df), from = 0, to = max(muestra_final), 
...
cat("✔ Simulación completada. Se observa convergencia de las medias y varianzas hacia sus valores teóricos.\n")
```
...
```{r}
# --- Verificación del Teorema Central del Límite para χ² ---

set.seed(123)

# Parámetro de la distribución χ²
...
for (n in n_values) {
  ...
  # --- 1. Generar medias muestrales ---
  medias <- replicate(num_sim, mean(rchisq(n, df = df)))
  
  # --- 2. Parámetros de la normal teórica según TCL ---
  mu <- df
  sigma <- sqrt(2 * df / n)
  
  # --- 3. Graficar histograma de las medias muestrales ---
  hist(medias, breaks = 40, probability = TRUE, col = "lightblue",
       main = paste("n =", n),
       xlab = "Media muestral", ylab = "Densidad")
  
  # Curva normal teórica
  curve(dnorm(x, mean = mu, sd = sigma), from = min(medias), to = max(medias),
        add = TRUE, col = "red", lwd = 2)
  
  legend("topright", legend = c("Simulación", "Normal teórica"),
         col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")
  
  # --- 4. Mostrar valores de media y varianza empírica ---
  cat("\nTamaño de muestra n =", n, "\n")
  cat("Media de las medias muestrales:", round(mean(medias), 4), "\n")
  cat("Varianza de las medias muestrales:", round(var(medias), 4), "\n")
  cat("Media teórica:", mu, "\n")
  cat("Varianza teórica (Var(X)/n):", round(2*df/n, 4), "\n")
}
...
```{r}
# --- Verificación de la propiedad de aditividad de χ² ---

set.seed(123)

# Parámetros
n_values <- c(5, 10, 20)   # grados de libertad equivalentes al número de normales al cuadrado
num_sim <- 10000            # número de simulaciones
...
for (n in n_values) {
  ...
  # --- 1. Generar sumas de cuadrados de normales estándar ---
  Y <- replicate(num_sim, sum(rnorm(n)^2))
  
  # --- 2. Graficar histograma de las sumas ---
  hist(Y, breaks = 40, probability = TRUE, col = "lightblue",
       main = bquote(paste("Suma de", .(n), "N(0,1)^2")),
       xlab = "Y = sum(X_i^2)", ylab = "Densidad")
  
  # --- 3. Superponer densidad χ² teórica ---
  curve(dchisq(x, df = n), from = min(Y), to = max(Y), 
        add = TRUE, col = "red", lwd = 2)
  
  legend("topright", legend = c("Simulación", "χ² teórica"),
         col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")
  
  # --- 4. Mostrar media y varianza empíricas ---
  cat("\nGrados de libertad n =", n, "\n")
  cat("Media simulada:", round(mean(Y), 4), "vs media teórica:", n, "\n")
  cat("Varianza simulada:", round(var(Y), 4), "vs varianza teórica:", 2*n, "\n")
}
...
```{r}
# --- Aproximación normal de la distribución χ² ---

set.seed(123)

# Valores de grados de libertad
n_values <- c(10, 30, 50, 100)
num_sim <- 10000  # número de simulaciones por valor de n
...
for (n in n_values) {
  ...
  # --- 1. Generar muestras χ²(n) ---
  X <- rchisq(num_sim, df = n)
  
  # --- 2. Parámetros de la normal aproximada ---
  mu <- n
  sigma <- sqrt(2 * n)
  
  # --- 3. Graficar histograma de la muestra ---
  hist(X, breaks = 50, probability = TRUE, col = "lightblue",
       main = paste("χ²(", n, ") vs Normal aproximada"),
       xlab = "X", ylab = "Densidad")
  
  # --- 4. Superponer densidad normal teórica ---
  curve(dnorm(x, mean = mu, sd = sigma), from = min(X), to = max(X),
        add = TRUE, col = "red", lwd = 2)
  
  legend("topright", legend = c("Simulación χ²", "Normal aproximada"),
         col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")
  
  # --- 5. Mostrar media y varianza simuladas ---
  cat("\nGrados de libertad n =", n, "\n")
  cat("Media simulada:", round(mean(X), 4), "vs media teórica:", mu, "\n")
  cat("Varianza simulada:", round(var(X), 4), "vs varianza teórica:", sigma^2, "\n")
}
...
```{r}
# --- Verificación de la aditividad de χ² ---
...
# --- 1. Generar variables χ² independientes ---
X <- rchisq(num_sim, df = n1)
Y <- rchisq(num_sim, df = n2)

# --- 2. Sumar las variables ---
Z <- X + Y

# --- 3. Graficar histograma de la suma ---
hist(Z, breaks = 40, probability = TRUE, col = "lightblue",
     main = bquote(paste("Z = X + Y, ", chi^2, "(", .(n1+n2), ")")),
     xlab = "Z", ylab = "Densidad")

# --- 4. Superponer densidad χ² teórica ---
curve(dchisq(x, df = n1+n2), from = min(Z), to = max(Z),
      add = TRUE, col = "red", lwd = 2)

legend("topright", legend = c("Simulación", bquote(chi^2~"teórica")),
       col = c("lightblue", "red"), lwd = c(10,2), bty = "n")

# --- 5. Mostrar media y varianza simuladas ---
cat("Suma de χ² independientes:\n")
cat("Media simulada:", round(mean(Z), 4), "vs teórica:", n1+n2, "\n")
cat("Varianza simulada:", round(var(Z), 4), "vs teórica:", 2*(n1+n2), "\n")
