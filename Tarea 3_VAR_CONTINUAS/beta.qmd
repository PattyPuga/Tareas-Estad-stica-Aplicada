...
```{r}
# --- Simulación y Visualización de una distribución Beta(2,5) ---
...
varianzas_muestrales <- numeric(length(tamanos))

# Simulación
set.seed(123)  # Para reproducibilidad
for (i in seq_along(tamanos)) {
  muestra <- rbeta(tamanos[i], a, b)
  medias_muestrales[i] <- mean(muestra)
  varianzas_muestrales[i] <- var(muestra)
}

# --- 1. Gráfica de convergencia de la media ---
plot(tamanos, medias_muestrales, type = "l", col = "blue",
     main = "Convergencia de la Media Muestral a la Media Teórica",
...
plot(tamanos, varianzas_muestrales, type = "l", col = "darkgreen",
     main = "Convergencia de la Varianza Muestral a la Varianza Teórica",
     xlab = "Tamaño de la muestra", ylab = "Varianza muestral")
abline(h = var_teorica, col = "red", lwd = 2, lty = 2)
legend("topright", legend = c("Varianza muestral", "Varianza teórica"),
       col = c("darkgreen", "red"), lty = c(1,2), bty = "n")

# --- 3. Histograma con la PDF teórica superpuesta ---
n_max <- max(tamanos)
muestra_final <- rbeta(n_max, a, b)

hist(muestra_final, breaks = 30, probability = TRUE, col = "lightblue",
     main = paste("Histograma y PDF Teórica - n =", n_max),
     xlab = "x", ylab = "Densidad")

curve(dbeta(x, a, b), from = 0, to = 1, add = TRUE, col = "red", lwd = 2)
legend("topright", legend = c("Histograma", "PDF Teórica"),
...
```{r}
# --- Teorema Central del Límite con distribución Beta(2,5) ---
...
par(mfrow = c(1, length(tamanos)))
set.seed(123)

for (n in tamanos) {
  # Generar muchas medias muestrales
  medias <- replicate(num_sim, mean(rbeta(n, a, b)))
  
  # Ajuste normal según TCL
  mu <- media_teorica
  ...
  legend("topright", legend = c("Media muestral", "Normal teórica"),
         col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")
}

# --- Explicación ---
cat("El Teorema Central del Límite indica que, al aumentar el tamaño de muestra (n),\n",
    "la distribución de las medias muestrales tiende a una Normal con:\n",
    "media =", round(media_teorica, 3),
    "y desviación estándar =", round(sqrt(var_teorica / max(tamanos)), 4), "\n")
