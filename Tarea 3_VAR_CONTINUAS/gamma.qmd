...
```{r}
# --- Simulación de distribución Gamma y verificación del TCL ---

set.seed(123)

# Parámetros de la distribución Gamma
alpha <- 2.5      # parámetro de forma
lambda <- 1.8     # parámetro de tasa
...
# --- 1. Simulación ---
for (n in n_values) {
  # Generar 1000 muestras y calcular medias
  medias <- replicate(num_sim, mean(rgamma(n, shape = alpha, rate = lambda)))
  
  # Guardar resultados
  medias_muestrales[[as.character(n)]] <- medias
  
  # Calcular media y varianza empíricas de las medias muestrales
  media_emp <- mean(medias)
  var_emp <- var(medias)
  
  # Varianza teórica de la media muestral = var_teorica / n
  var_teo_muestral <- var_teorica / n
  
  # Guardar en tabla
  resultados <- rbind(resultados, 
                      data.frame(
                        n = n,
                        Media_Muestral = round(media_emp, 4),
                        Media_Teórica = round(media_teorica, 4),
                        Var_Muestral = round(var_emp, 4),
                        Var_Teórica_div_n = round(var_teo_muestral, 4)
                      ))
}

# --- 2. Mostrar tabla de comparación ---
cat("Comparación de medias y varianzas:\n")
print(resultados)

# --- 3. Graficar histogramas con densidad normal teórica superpuesta ---
par(mfrow = c(2, 3))  # 6 gráficos

for (n in n_values) {
  medias <- medias_muestrales[[as.character(n)]]
  
  # Parámetros de la normal teórica según el TCL
  mu <- media_teorica
  sigma <- sqrt(var_teorica / n)
  ...
  curve(dnorm(x, mean = mu, sd = sigma),
        from = min(medias), to = max(medias),
        add = TRUE, col = "red", lwd = 2)
  
  legend("topright", legend = c("Simulación", "Normal teórica (TCL)"),
         col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")
}

# --- 4. Conclusión ---
cat("\nConclusión:\n")
cat("✔ Las medias muestrales convergen hacia la media teórica =", round(media_teorica, 3), "\n")
cat("✔ La varianza de las medias muestrales se aproxima a Var(X)/n.\n")
cat("✔ Conforme aumenta n, las distribuciones de las medias se aproximan a una Normal, confirmando el TCL.\n")
...
```{r}
# --- Suma de variables exponenciales y relación con la distribución Gamma ---
...
# Configurar el área de gráficos
par(mfrow = c(1, 3))  

for (n in n_values) {
  # --- 1. Generar 1000 sumas de n variables exponenciales ---
  sumas <- replicate(num_sim, sum(rexp(n, rate = lambda)))
  
  # --- 2. Graficar histograma ---
  hist(sumas, breaks = 30, probability = TRUE, 
       col = "lightblue", border = "white",
       main = paste("Suma de", n, "Exp(λ=1.5)"),
       xlab = "Y = suma de X_i", ylab = "Densidad")
  
  # --- 3. Superponer la densidad teórica Gamma(n, λ) ---
  x_vals <- seq(min(sumas), max(sumas), length.out = 300)
  lines(x_vals, dgamma(x_vals, shape = n, rate = lambda), 
        col = "red", lwd = 2)
  
  legend("topright", legend = c("Simulación", "Gamma teórica"),
         col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")
}

# --- 4. Conclusión ---
cat("✔ Se observa que la suma de n variables Exp(λ) sigue aproximadamente una Gamma(n, λ).\n")
cat("✔ Al aumentar n, la distribución se vuelve más simétrica y parecida a una Normal (por el TCL).\n")
