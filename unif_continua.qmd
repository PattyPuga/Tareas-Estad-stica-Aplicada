...
```{r}
# --- Análisis de Tiempo de Carga (Distribución Uniforme) ---
...
# --- 1. Simulación de tiempos de carga ---
tiempos <- runif(n, min = min_t, max = max_t)

# Histograma con densidad teórica
hist(tiempos, breaks = 30, probability = TRUE, col = "lightblue",
     main = "Simulación de Tiempos de Carga",
     xlab = "Tiempo de carga (s)", ylab = "Densidad")

# Densidad teórica de la uniforme
curve(dunif(x, min = min_t, max = max_t), from = min_t, to = max_t,
      add = TRUE, col = "red", lwd = 2)
legend("topright", legend = c("Simulación", "Densidad teórica"),
       col = c("lightblue", "red"), lwd = 2)

# --- 2. Percentil 75 ---
percentil_75 <- qunif(0.75, min = min_t, max = max_t)
cat("Percentil 75 de los tiempos de carga:", round(percentil_75, 2), "segundos\n")

# --- 3. Probabilidad de cargar en menos de 4 segundos ---
prob_menor4 <- punif(4, min = min_t, max = max_t)
cat("Probabilidad de que la página cargue en menos de 4 segundos:", round(prob_menor4, 4), "\n")
...
```{r}
# --- Distribución de Calificaciones (Uniforme) ---
...
calificaciones <- runif(n, min = min_cal, max = max_cal)

# --- 2. Porcentaje de estudiantes con calificación > 85 ---
porc_mayor85 <- mean(calificaciones > 85) * 100
cat("Porcentaje de estudiantes con calificación > 85:", round(porc_mayor85, 2), "%\n")
...
porc_categorias <- prop.table(table(categorias)) * 100
cat("Porcentaje de estudiantes por categoría:\n")
print(round(porc_categorias,2))

# --- 4. Porcentajes teóricos ---
# Longitud de intervalos sobre total del rango (100-70=30)
teoricos <- c((79-70)/30, (89-79)/30, (100-89)/30) * 100
names(teoricos) <- c("Aprobado","Bien","Excelente")
cat("Porcentajes teóricos por categoría:\n")
print(round(teoricos,2))

# --- 5. Graficar la distribución con categorías ---
hist(calificaciones, breaks = seq(70,100,1), col = "lightblue",
     main = "Distribución de Calificaciones",
     xlab = "Calificación", ylab = "Frecuencia")

abline(v = c(79, 89), col = "red", lwd = 2, lty = 2)
text(x = c(74.5, 84, 94.5), y = max(table(calificaciones))/2,
     labels = c("Aprobado","Bien","Excelente"), col = "darkblue", cex = 0.9)
...
```{r}
# --- TCL con distribución uniforme truncada ---

set.seed(123)
library(moments)  # para skewness y kurtosis

# --- 1. Generar distribución uniforme truncada ---
n_total <- 10000
x <- runif(n_total, 0, 1)
x_trunc <- x[x > 0.3]  # conservar solo valores mayores a 0.3
hist(x_trunc, breaks = 30, col = "lightblue", main = "Distribución Uniforme Truncada (>0.3)",
     xlab = "Valores", ylab = "Frecuencia")

# --- 2. Verificar TCL para diferentes tamaños de muestra ---
n_values <- c(5, 15, 30, 60, 120)
num_muestras <- 800

par(mfrow = c(3,2))  # para mostrar los histogramas

for (n in n_values) {
  
  medias_muestrales <- replicate(num_muestras, mean(sample(x_trunc, n, replace = TRUE)))
  
  # Histograma de medias normalizado
  hist(medias_muestrales, breaks = 30, probability = TRUE, col = "lightgreen",
       main = paste("TCL - n =", n), xlab = "Media muestral", ylab = "Densidad")
  
  # Superponer densidad normal teórica
  mu_med <- mean(x_trunc)
  sigma_med <- sd(x_trunc)/sqrt(n)
  curve(dnorm(x, mean = mu_med, sd = sigma_med), add = TRUE, col = "red", lwd = 2)
  
  # Skewness y kurtosis
  skw <- skewness(medias_muestrales)
  krt <- kurtosis(medias_muestrales)
  cat("\nTamaño de muestra n =", n, "\n")
  cat("Skewness de medias:", round(skw, 4), "\n")
  cat("Kurtosis de medias:", round(krt, 4), "\n")
}
...
# Generar puntos uniformes en [-1,1] x [-1,1]
x <- runif(n, -1, 1)
y <- runif(n, -1, 1)

# Determinar cuántos puntos caen dentro del círculo de radio 1
dentro_circulo <- x^2 + y^2 <= 1

# Estimar pi
pi_est <- 4 * mean(dentro_circulo)
cat("Estimación de pi usando Monte Carlo:", round(pi_est, 6), "\n")

# --- Visualización ---
plot(x[dentro_circulo], y[dentro_circulo], col = "blue", pch = 16, cex = 0.5,
     xlab = "x", ylab = "y", asp = 1, main = "Estimación de pi con Monte Carlo")
points(x[!dentro_circulo], y[!dentro_circulo], col = "red", pch = 16, cex = 0.5)
legend("topright", legend = c("Dentro del círculo", "Fuera del círculo"),
