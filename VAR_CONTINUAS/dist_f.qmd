...
```{r}
# --- Simulación numérica de propiedades entre distribuciones t, F y χ² ---

set.seed(123)
par(mfrow = c(1, 3))  # 3 gráficas en una fila

# ============================================================
# a) Si X ~ t(n) entonces X^2 ~ F(1, n)
# ============================================================

n <- 10                      # grados de libertad
muestras_t <- rt(100000, df = n)
muestras_f <- muestras_t^2   # X^2

# Histograma comparado con F(1,n)
hist(muestras_f, breaks = 50, probability = TRUE, col = "lightblue",
     main = expression(paste("(a)  Si  X ~ t(n)  =>  X"^2," ~ F(1,n)")),
     xlab = "x", xlim = c(0, 10))
curve(df(x, df1 = 1, df2 = n), col = "red", lwd = 2, add = TRUE)
legend("topright", legend = c("Simulación", "F(1,n) teórica"),
       col = c("lightblue", "red"), lwd = c(10, 2), bty = "n")

# ============================================================
# b) Si X ~ F(a,b) entonces 1/X ~ F(b,a)
# ============================================================

a <- 5
b <- 10
muestras_F <- rf(100000, df1 = a, df2 = b)
muestras_inv <- 1 / muestras_F  # 1/X

# Histograma comparado con F(b,a)
hist(muestras_inv, breaks = 50, probability = TRUE, col = "lightgreen",
     main = expression(paste("(b)  Si  X ~ F(a,b)  =>  1/X ~ F(b,a)")),
     xlab = "x", xlim = c(0, 5))
curve(df(x, df1 = b, df2 = a), col = "red", lwd = 2, add = TRUE)
legend("topright", legend = c("Simulación", "F(b,a) teórica"),
       col = c("lightgreen", "red"), lwd = c(10, 2), bty = "n")

# ============================================================
# c) Si X ~ χ²(a) y Y ~ χ²(b) independientes, entonces (X/a)/(Y/b) ~ F(a,b)
# ============================================================

a <- 4
b <- 8
X <- rchisq(100000, df = a)
Y <- rchisq(100000, df = b)
muestras_F2 <- (X / a) / (Y / b)

# Histograma comparado con F(a,b)
hist(muestras_F2, breaks = 50, probability = TRUE, col = "lightcoral",
     main = expression(paste("(c)  (X/a)/(Y/b)  ~  F(a,b)")),
     xlab = "x", xlim = c(0, 5))
curve(df(x, df1 = a, df2 = b), col = "red", lwd = 2, add = TRUE)
legend("topright", legend = c("Simulación", "F(a,b) teórica"),
       col = c("lightcoral", "red"), lwd = c(10, 2), bty = "n")

# ============================================================
# --- Mensaje final ---
cat("✔ Verificación completada:\n",
    "a) X^2 de t(n) se ajusta a F(1,n)\n",
    "b) 1/X de F(a,b) se ajusta a F(b,a)\n",
    "c) (X/a)/(Y/b) con X~χ²(a), Y~χ²(b) se ajusta a F(a,b)\n")
