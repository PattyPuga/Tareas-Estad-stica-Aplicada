...
```{r}
neg_binomial_comparacion <- function(r, p, n_sim = 10000) {
  set.seed(123)
  
  # --- Suma de r variables geométricas ---
  # Geométrica en R: número de fracasos antes del primer éxito: rgeom()
  suma_geom <- replicate(n_sim, sum(rgeom(r, prob = p)))
  
  # --- Binomial Negativa: número de fracasos hasta r-ésimo éxito ---
  # En R, rnbinom(n, size=r, prob=p) genera esto directamente
  binom_neg <- rnbinom(n_sim, size = r, prob = p)
  
  # --- Graficar histogramas comparativos ---
  hist(suma_geom, breaks = seq(min(suma_geom)-0.5, max(suma_geom)+0.5, 1),
       probability = TRUE, col = rgb(0,0,1,0.5), 
       main = paste("Suma de Geométricas vs Binomial Negativa (r =", r, ", p =", p, ")"),
       xlab = "Número de fracasos", ylab = "Probabilidad")
  
  hist(binom_neg, breaks = seq(min(binom_neg)-0.5, max(binom_neg)+0.5, 1),
       probability = TRUE, col = rgb(1,0,0,0.5), add = TRUE)
  
  legend("topright", legend = c("Suma de geométricas", "Binomial negativa"),
         fill = c(rgb(0,0,1,0.5), rgb(1,0,0,0.5)))
  
  # --- Valores teóricos (opcional) ---
  x_vals <- 0:max(suma_geom)
  lines(x_vals, dnbinom(x_vals, size = r, prob = p), col="darkgreen", lwd=2)
  legend("topleft", legend=c("PMF teórica Binomial Negativa"), col="darkgreen", lwd=2)
  
  # Retornar lista con resultados
  return(list(suma_geom=suma_geom, binom_neg=binom_neg))
}

# --- Ejemplo de uso ---
resultado <- neg_binomial_comparacion(r = 5, p = 0.3, n_sim = 10000)
...
# --- Simulación: número de fallos antes de 8 tiros libres exitosos ---
fallos <- rnbinom(n_sim, size = r, prob = p)

# --- Estadísticas ---
media_sim <- mean(fallos)
cat("Número esperado de tiros fallados (teórico):", round(r*(1-p)/p,4), "\n")
cat("Número promedio de tiros fallados en simulación:", round(media_sim,4), "\n")

# --- Histograma comparativo con PMF teórica ---
hist(fallos, breaks = (min(fallos):(max(fallos)+1))-0.5, probability = TRUE,
     col = "lightblue", main = "Tiros fallados antes de 8 éxitos", xlab = "Número de fallos")

x_vals <- 0:max(fallos)
lines(x_vals, dnbinom(x_vals, size=r, prob=p), col="red", lwd=2)
legend("topright", legend=c("Simulación","Binomial Negativa teórica"),
...
# a) Probabilidad de exactamente 12 fallos
prob_12 <- dnbinom(12, size=r, prob=p)
cat("a) Probabilidad de exactamente 12 fallos:", round(prob_12,6), "\n")

# b) Probabilidad de más de 20 fallos
prob_mas_20 <- 1 - pnbinom(20, size=r, prob=p)
cat("b) Probabilidad de más de 20 fallos:", round(prob_mas_20,6), "\n")

# c) Costo esperado en pruebas fallidas
esperado_fallos <- r * (1-p)/p
costo_esperado <- esperado_fallos * costo_por_prueba
cat("c) Costo esperado en pruebas fallidas: $", round(costo_esperado,2), "\n")
