...
prob_par <- sum(bolas %% 2 == 0) / length(bolas)
...
# Frecuencias observadas
freq_obs <- table(muestra)/n

# Frecuencias teóricas
freq_teo <- rep(1/length(valores), length(valores))
names(freq_teo) <- valores

# Mostrar comparación
df <- data.frame(Valor=valores,
                 Frec_Observada=as.numeric(freq_obs),
                 Frec_Teorica=as.numeric(freq_teo))
print(df)

# Gráfico comparativo
barplot(rbind(freq_obs, freq_teo), beside=TRUE,
        col=c("lightblue","red"), legend=c("Observada","Teórica"),
        main="Frecuencias Observadas vs Teóricas",
