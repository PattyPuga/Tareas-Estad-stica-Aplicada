...
```{r}
# --- Análisis de la distribución t de Student (5 grados de libertad) ---
...
# Valores teóricos
media_teorica <- 0
var_teorica <- gl / (gl - 2)  # para gl > 2

# --- 1. Cálculo de medias y varianzas muestrales ---
cat("Comparación de medias y varianzas muestrales vs teóricas:\n\n")
...
for (n in tamanos) {
  ...
  resultados <- rbind(resultados, 
                      data.frame(Tamaño = n,
                                 Media_Muestral = round(media_m, 4),
                                 Varianza_Muestral = round(var_m, 4),
                                 Media_Teórica = media_teorica,
                                 Var_Teórica = round(var_teorica, 4)))
}

print(resultados)

# --- 2. Histograma y comparación con densidades teóricas ---
n_max <- max(tamanos)
muestra_grande <- rt(n_max, df = gl)

hist(muestra_grande, breaks = 30, probability = TRUE, col = "lightblue",
     main = paste("Distribución t(5) vs Normal(0,1) — n =", n_max),
     xlab = "x", ylab = "Densidad")

# Densidad t(5)
curve(dt(x, df = gl), from = -5, to = 5, add = TRUE, col = "red", lwd = 2)

# Densidad Normal estándar
curve(dnorm(x, mean = 0, sd = 1), from = -5, to = 5, add = TRUE, col = "darkgreen", lwd = 2, lty = 2)

legend("topright",
       legend = c("Muestra simulada", "t(5) teórica", "Normal(0,1)"),
       col = c("lightblue", "red", "darkgreen"),
       lwd = c(10, 2, 2), lty = c(1, 1, 2), bty = "n")

# --- Nota final ---
cat("\nInterpretación:\n")
cat("✔ Las medias muestrales se aproximan a 0 (media teórica).\n")
cat("✔ Las varianzas muestrales se aproximan a", round(var_teorica, 3), "(varianza teórica).\n")
cat("✔ En la gráfica, la t(5) tiene colas más pesadas que la Normal(0,1),\n",
    "pero con n grande su forma se aproxima a la Normal.\n")
...
```{r}
# --- Generación y verificación de la variable T = X / sqrt(Y/n) ~ t(n) ---
...
# 1. Generar las variables independientes
X <- rnorm(N, mean = 0, sd = 1)   # X ~ N(0,1)
Y <- rchisq(N, df = n)            # Y ~ χ²(n)

# 2. Calcular la variable T
T <- X / sqrt(Y / n)

# 3. Graficar el histograma y la densidad teórica
hist(T, breaks = 30, probability = TRUE, col = "lightblue",
     main = expression(paste("Simulación de  T = X / sqrt(Y/n)  con  n = 10")),
     xlab = "T", ylab = "Densidad")

# Superponer la densidad teórica de t(10)
curve(dt(x, df = n), from = -5, to = 5, add = TRUE, col = "red", lwd = 2)

legend("topright",
       legend = c("Simulación", "t(10) teórica"),
       col = c("lightblue", "red"),
       lwd = c(10, 2), bty = "n")

# --- 4. Comprobación estadística ---
cat("Comprobación numérica:\n")
cat("Media muestral de T:", round(mean(T), 4), "\n")
cat("Varianza muestral de T:", round(var(T), 4), "\n")
cat("Media teórica: 0\n")
cat("Varianza teórica: n / (n - 2) =", round(n / (n - 2), 4), "\n")
...
```{r}
# --- Verificación del estadístico t de Student por simulación ---
...
# --- 2. Visualización ---
hist(T_values, breaks = 40, probability = TRUE, col = "lightblue",
     main = expression(paste("Simulación del estadístico  T = (X̄ - μ) / (S / √n)")),
     xlab = "T", ylab = "Densidad")

# Superponer la densidad teórica t(n-1)
curve(dt(x, df = n - 1), from = -5, to = 5, add = TRUE, col = "red", lwd = 2)

legend("topright",
       legend = c("Simulación", "t(n-1) teórica"),
       col = c("lightblue", "red"),
       lwd = c(10, 2), bty = "n")

# --- 3. Comprobación numérica ---
cat("Comprobación estadística:\n")
cat("Media muestral de T:", round(mean(T_values), 4), "\n")
cat("Varianza muestral de T:", round(var(T_values), 4), "\n")
cat("Media teórica: 0\n")
cat("Varianza teórica: (n-1)/(n-3) =", round((n - 1) / (n - 3), 4), " (válido para n>3)\n")
...
```{r}
# --- Simulación del estadístico T para verificar distribución t(9) ---
...
num_muestras <- 1000  # número de simulaciones

# --- 1. Generar 1000 muestras y calcular T ---
T_values <- replicate(num_muestras, {
  # Generar muestra normal estándar
...
# --- 2. Graficar histograma y comparar con la densidad teórica t(9) ---
hist(T_values, breaks = 30, probability = TRUE, col = "lightblue",
     main = expression(paste("Distribución del estadístico  T = (X̄ - μ) / (S / √n),  n=10")),
     xlab = "T", ylab = "Densidad")

# Superponer la densidad teórica t(9)
curve(dt(x, df = n - 1), from = -5, to = 5, add = TRUE, col = "red", lwd = 2)

legend("topright",
       legend = c("Simulación", "t(9) teórica"),
       col = c("lightblue", "red"),
       lwd = c(10, 2),
       bty = "n")

# --- 3. Comprobación numérica ---
cat("Comprobación de la simulación:\n")
cat("Media muestral de T:", round(mean(T_values), 4), "\n")
cat("Varianza muestral de T:", round(var(T_values), 4), "\n")
cat("Media teórica: 0\n")
cat("Varianza teórica de t(9):", round((n - 1) / (n - 3), 4), "\n")
